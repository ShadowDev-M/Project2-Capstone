...
MeshComponent::MeshComponent(Component *parent_, const char* filename_, bool renderShadows_): Component(parent_), filename(filename_), renderShadows(renderShadows_) {}
...
void MeshComponent::LoadModel(const char* filename) {
    ...
    const aiScene* scene = importer.ReadFile(pathToOpen, aiProcessPreset_TargetRealtime_Fast);
...
void MeshComponent::StoreMeshData(GLenum drawmode_) {
    ...
    if (skeleton) {
        ...
        size_t totalSize = vertices.size() * sizeof(Vec3) +
            ...
            tangents.size() * sizeof(Vec3);  // SIMPLIFIED
